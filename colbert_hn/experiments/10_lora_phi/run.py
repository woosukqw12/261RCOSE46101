"""10_lora_phi — LoRA adapter 의 ColBERT v2 encoder 적용.
...
def main() -> None:
    ...
    tag = f"{''.join(components)}_r{args.r}_l{len(layers) if layers else 12}"
    if args.tag_suffix:
        tag = f"{tag}_{args.tag_suffix}"
    base_out = artifact_dir(exp_name="10_lora_phi", dataset=args.dataset, seed=args.seed)
    out = base_out / tag
