...
class RouterConfig:
    ...
    # harmfulness = alpha * margin + beta * volatility + gamma * pos_neg_sim
    alpha: float = 1.0
    beta: float = 0.2
    gamma: float = 0.3
...
class DynamicRouter:
    ...
    @torch.no_grad()
    def update(self, qids, neg_ids, pos_scores, neg_scores, pos_neg_sims):
        ...
        for i in range(B):
            ...
            for j in range(K):
                ...
                pn_sim = float(pos_neg_sims[i, j].item())

                raw_h = (
                    self.cfg.alpha * margin
                    + self.cfg.beta * volatility
                    + self.cfg.gamma * pn_sim
                )
    ...
    for epoch in range(args.epochs):
        ...
        for batch in pbar:
            ...
            with torch.no_grad():
                pos_scores = torch.sum(q_emb * p_emb, dim=-1)
                neg_scores = torch.einsum("bd,bkd->bk", q_emb, n_emb)
                pos_neg_sims = torch.einsum("bd,bkd->bk", p_emb, n_emb)

                router.update(qids, neg_ids, pos_scores, neg_scores, pos_neg_sims)
            ...
            pn_mean = float(pos_neg_sims.mean().item())
            pn_min = float(pos_neg_sims.min().item())
            pn_max = float(pos_neg_sims.max().item())
        ...
        if epoch_router_stats:
            metrics_all["router"] = {
                ...
                "pn_mean": sum(x["pn_mean"] for x in epoch_router_stats) / len(epoch_router_stats),
                "pn_min": min(x["pn_min"] for x in epoch_router_stats),
                "pn_max": max(x["pn_max"] for x in epoch_router_stats),
            }
...
def build_argparser():
    ...
    ap.add_argument("--tau_safe", type=float, default=0.00)
    ap.add_argument("--tau_amb", type=float, default=0.05)
    ap.add_argument("--tau_susp", type=float, default=0.12)
